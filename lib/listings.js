...
/**
 * Fetch a single published item by slug from a DRF detail endpoint.
 * Returns null on any failure so pages can render a graceful not-found.
 */
export async function getPublishedDetail(endpoint, slug, { revalidate = 300 } = {}) {
  try {
    ...
    console.error(`getPublishedDetail(${endpoint}/${slug}) error:`, error.message);
    return null;
  }
}
